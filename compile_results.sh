...
for MODEL in $(ls $TEST_FOLDER) ; do
	for Q in $(seq 1 16) ; do
		...
		UOUT=$([[ -f $RED_RES_FILE ]] && cat "$COL_RED_RES_FILE" | grep -v "^<"  | grep -v "^Query before" | grep -v "^Query after" | tr '\n' '\r' || echo "")
		ROUT=$([[ -f $RED_RES_FILE ]] && cat "$RED_RES_FILE" | grep -v "^<"  | grep -v "^Query before" | grep -v "^Query after" | tr '\n' '\r' || echo "")
		VOUT=$([[ -f $VERI_RES_FILE ]] && cat $VERI_RES_FILE | grep -v "^<"  | grep -v "^Query before" | grep -v "^Query after" | tr '\n' '\r' || echo "@@@0,0@@@")
