...
if __name__ == "__main__":
    ...
    for index, options in enumerate(options_list):
        ...
        test_name = "{:<25}".format(folder) + "{:<25}".format(options.category) + "{:<10}".format(
            options.search_strategy) + "{:<20}".format(model_folder)
        progress = f"{index}/{len(options_list)}"
        progress = "{:<7}".format(progress)
        if len(options.results_to_plot) == 0:
            if "inhib" in model_folder or folder != 'CPN-4-30-4-2-ioless':
